...
File Path:   addons/wcd/lua/wcd/client/wcd_admin_ui.lua
...
function WCD:OpenAdmin(data)
	if IsValid(self.AdminUI) then self.AdminUI:Remove() end
	...
	frame.title:SetPos(ScrW() / 2 - frame.title:GetWide() / 2, ScrH() / 2 + frame:GetTall() / 2)
	function frame.title:Paint(w, h)
		draw.RoundedBoxEx(16, 0, 0, w, h, c.frameBg, false, false, true, true)
		draw.SimpleTextOutlined("WCD::Administration", "WCD::FontFrameSubTitle", w / 2, h / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
	end
	...
	function frame.close:Think()
		if !IsValid(frame) then self:Remove() end
	end
	...
	for i, v in pairs(lang.content) do
		...
		if v.key == "header" then
			...
			q.label:SetPos(q:GetWide() / 2 - q.label:GetWide() / 2, q:GetTall() - q.label:GetTall())
			continue
		end
	...
	function a.bottom.save:DoClick()
		...
		for i, v in pairs(a.settingsList) do
			...
			if v.choice.wrong then
				WCD:Notification(WCD:Translate("invalidSettingValue", (v.label and v.label:GetText()) or i), WCD.AdminUI)
				return
			end

			if v.default and v:GetValue() != v.default or v:GetValue() != WCD.Settings[i] then send[i] = v:GetValue() end
		end
		...
		print("sending these updated settings:")
		PrintTable(send)
	...
	a.top.orText:SetPos(w / 2 - a.top.orText:GetWide() / 2, a.top:GetTall() / 2 - a.top.orText:GetTall() / 2)
	a.top.editVehicle = a:Add("WCD::DComboBox")
	a.top.editVehicle:SetSize(a.top.selectVehicle:GetSize())
	a.top.editVehicle:SetPos(w - a.top.editVehicle:GetWide(), 5)
	function a:RebuildTop()
		...
		for i, v in pairs(WCD.VehicleData or {}) do
			local name = v.Name
			if WCD.VehicleClassCounter[i] then name = name .. WCD:Translate(lang.activeCount, WCD.VehicleClassCounter[i]) end
			if v.__WCDEnt then
				if v.cat then
					name = "z[" .. v.cat .. "] " .. name
				else
	...
	a.top.editButton:SetPos(w - a.top.editVehicle:GetWide(), a.top:GetTall() - a.top.editButton:GetTall())
	...
	a.top.deleteButton:SetPos(w - a.top.deleteButton:GetWide(), a.top:GetTall() - a.top.deleteButton:GetTall())
	...
	function a:Build(editing, class)
		...
		for i, v in pairs(lang.content) do
			if data.__WCDEnt and v.notForEnt and (!WCD.Settings.allowEntityCustomization or v.neverForEnt) then continue end
			...
			elseif v.type == "number" or v.type == "string" then
				q.choice = q:Add("WCD::DTextEntry")
				q.choice:SetSize(92, 24)
				if v.type == "string" then q.choice:SetWide(250) end
				q.choice:SetPos(q:GetWide() - q.choice:GetWide() - 1, q:GetTall() / 2 - q.choice:GetTall() / 2)
		...
		function a.bottom.save:DoClick()
			...
			data.class = (editing and ref and ref.GetClass and ref:GetClass()) or class
			data.id = (editing and ref and ref.GetId and ref:GetId()) or -1
	...
	a.top.inputSteam:SetPos(w - a.top.inputSteam:GetWide(), 5)
	function a:RebuildTop()
	...
	a.top.inputSteamButton:SetPos(w - a.top.inputSteamButton:GetWide(), a.top:GetTall() - a.top.inputSteamButton:GetTall())
	...
	function a:Build(value, data)
		...
		if data then
			...
			if !target then target = string.sub(value, 0, 20) end
			lastValue = value
			lastId = target
	...
	net.Receive("WCD::RequestPlayerVehicles", function() a:Build(lastValue, net.ReadTable()) end)
	a.bottom.save = a.bottom:Add("WCD::VariousButton")
	a.bottom.save:SetButtonColor(c.saveButton)
	a.bottom.save:SetFont("WCD::FontActionButton")
	a.bottom.save:SetNewText(WCD.Lang.save, true, 150)
...
hook.Add("OnPlayerChat", "WCD::OpenAdmin", function(ply, txt)
	if(ply == LocalPlayer() and txt == "!wcd" and WCD:IsOwner(ply)) then
...
timer.Simple(0, function() end) --WCD:OpenAdmin()
