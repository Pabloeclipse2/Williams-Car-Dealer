...
File Path:   addons/wcd/lua/wcd/client/wcd_vgui.lua
...
function panel:Paint(w, h)
	...
	if self:IsHovered() or self.alwaysHover then
	...
	if self.icon then
		...
		if self.iconRight then x = w - x - 16 end
	...
	if self.text != "" then
		...
		draw[action](self.text, self:GetFont(), w / 2, h / 2 - WCD.Settings.ButtonDepth / 4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
	end
end
...
function panel:Paint(w, h)
	...
	if self.icon then
		draw.SimpleTextOutlined(self.text or "", self:GetFont(), w / 2, h - 18, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
		surface.SetDrawColor(color_white)
		surface.SetMaterial(self.icon)
		if self.text != "" then
		...
		draw.SimpleTextOutlined(self.text or "", self:GetFont(), w / 2, h / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
	end
...
function panel:SetVehicle(id)
	...
	if not data then
		LocalPlayer():ChatPrint("TRIED SETTING UP A CAR THAT ISN'T IN WCD.LIST!")
		return
	end
	...
	if WCD.Client.Favorites and WCD.Client.Favorites[id] then self.star:SetFilled(true) end
	function self.star:DoClick()
		self:SetFilled(not self.filled)
...
function panel:Paint(w, h)
	if self:IsHovered() or self.star:IsHovered() or self.active then
		self.color = LerpVector(WCD.Settings.Lerp, self.color, self.hover)
	else
		self.color = LerpVector(WCD.Settings.Lerp, self.color, self.default)
	end
...
function panel:SetTrigger(panel, manual)
	self.trigger = panel
	if manual then self.manual = manual end
end

function panel:SetText(text)
	if not IsValid(self:GetParent()) then self:Remove() end
	surface.SetFont(self:GetFont())
	rows = string.Explode("\n", text)
	local tW, tH = surface.GetTextSize(text)
	oldTh = 18
	...
	if #rows > 1 then
		for i, v in pairs(rows) do
			...
			if tW < otherW then tW = otherW end
		end
	end
	...
	self.rows = rows
	self.oldTh = oldTh
	if not self.manual and self.trigger and IsValid(self.trigger) then
		self.trigger.OnCursorEntered = function() self:SetVisible(true) end
		self.trigger.OnCursorExited = function() self:SetVisible(false) end
	end
end
...
function panel:Paint(w, h)
	...
	if self.rows and table.Count(self.rows) > 1 then
...
function panel:Think()
	if self:IsVisible() and (not self.trigger or self.trigger:IsHovered()) then
...
function panel:OnGetFocus()
	self.focus = true
	if self.default and self:GetText() == self.default then self:SetText("") end
end

function panel:OnLoseFocus()
	self.focus = false
	if self.default and self:GetText() == "" then self:SetText(self.default) end
end
...
function panel:SetAllowed(valType, valMin, valMax)
	if valType == "number" then
		function self:IsAllowed()
			...
			if type(input) != "number" or input < (valMin or 0) or input > (valMax or 999999999) then
				self.wrong = true
			else
				self.wrong = false
			end
	...
	elseif valType == "string" then
		function self:IsAllowed()
			...
			if string.len(input) < 1 or string.len(input) > 120 then
				self.wrong = true
			else
				self.wrong = false
			end
...
function panel:OnChange()
	if self.IsAllowed then self:IsAllowed(self:GetText()) end
end

function panel:Paint(w, h)
	local show = false
	if self.focus or self:IsSelected() or self:IsHovered() then
		surface.SetDrawColor(color_white)
		show = true
	else
