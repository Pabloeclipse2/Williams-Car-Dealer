...
File Path:   addons/wcd/lua/wcd/client/wcd_various.lua
...
function WCD:GetSortedDealerTable(menuId, dealerGroup)
	...
	if not self.__TeamChange and LocalPlayer():Team() != self.TeamChange then self.__TeamChange = true end
	if self.__Change or self.__TeamChange then self.__NotYetSorted = {true, true, true, true} end
	if self.__CachedTables and self.__CachedTables[menuId] and not self.__NotYetSorted[menuId] then return self.__CachedTables[menuId] end
	local tbl = {}
	local new = {}
	if menuId == 1 then
	...
	for i, v in pairs(tbl) do
		if not self.List[i] or (dealerGroup and dealerGroup != self.List[i]:GetDealer()) then continue end
		table.insert(new, {
			id = i,
			price = (self.List[i]:GetFree() == true and 0) or self.List[i]:GetPrice() or 0
		})

		if self.Client.Settings.CacheOnReceive and not self.__Cached[self.VehicleData[self.List[i]:GetClass()].Model] then
	...
	if type(asc) != "boolean" then asc = true end
...
function WCD:OpenDealerHelper(current, parent, hideButtons)
	if self.DealerHelper and IsValid(self.DealerHelper) then self.DealerHelper:Remove() end
	...
	function a:Paint(w, h)
		...
		draw.SimpleTextOutlined("Dealer Helper", "WCD::FontFrameSubTitle", w / 2, 5, color_white, TEXT_ALIGN_CENTER, 0, 1, color_black)
	...
	q.title:SetPos(q:GetWide() / 2 - q.title:GetWide() / 2, q:GetTall() - q.title:GetTall())
	q.hover = 60
	...
	for i, v in pairs(WCD.DealerGroups) do
		...
		q.select:SetPos(q:GetWide() - q.select:GetWide(), q:GetTall() / 2 - q.select:GetTall() / 2)
		function q.select:DoClick()
			if parent and IsValid(parent) then parent.value = i end
		...
		if current and i == current then
			...
			q.select:SetPos(q:GetWide() - q.select:GetWide(), q:GetTall() / 2 - q.select:GetTall() / 2)
			function q.select:DoClick()
				if parent and IsValid(parent) then parent.value = nil end
				WCD:Notification(WCD.Lang.choiceSaved, WCD.AdminUI)
				a:Remove()
			end
		end
		...
		q.edit:SetPos(q:GetWide() - q.edit:GetWide() - q.select:GetWide() - 3, q:GetTall() / 2 - q.edit:GetTall() / 2)
		...
		q.delete:SetPos(q:GetWide() - q.delete:GetWide() - q.select:GetWide() - q.edit:GetWide() - 6, q:GetTall() / 2 - q.delete:GetTall() / 2)
		if hideButtons then
	...
	function a:Build(target)
		...
		function a.right.btn:DoClick()
			...
			if string.len(name) < 1 or string.len(name) > 90 or name == lang.newGroupName then
...
function WCD:OpenAccessHelper(current, parent)
	if self.AccessHelper and IsValid(self.AccessHelper) then
	...
	function a:Paint(w, h)
		surface.SetDrawColor(c.frameBg)
		surface.DrawRect(0, 0, w, h)
		draw.SimpleTextOutlined("Access Helper", "WCD::FontFrameSubTitle", w / 2, 5, color_white, TEXT_ALIGN_CENTER, 0, 1, color_black)
		surface.SetDrawColor(color_black)
		surface.DrawOutlinedRect(0, 0, w, h)
	end
	...
	function a:Build(target)
		a.right.items:Clear()
		local data = WCD.AccessGroups[target] or {
			name = "No name",
			jobs = {},
			ranks = {},
			needBoth = false
		...
		q.ranks:SetPos(q:GetWide() / 2 - q.ranks:GetWide() / 2, q:GetTall() - q.ranks:GetTall())
		q.hover = 60
		q.Paint = function(self, w, h) WCD.ListPaint(self, w, h) end
		...
		q.jobs:SetPos(q:GetWide() / 2 - q.jobs:GetWide() / 2, q:GetTall() - q.jobs:GetTall())
		q.hover = 60
		q.Paint = function(self, w, h) WCD.ListPaint(self, w, h) end
		...
		needBoth:SetPos(q:GetWide() - needBoth:GetWide() - 1, q:GetTall() - needBoth:GetTall() - 5)
		needBoth:SetChecked(data.needBoth or false)
		function needBoth:OnChange()
			if not needBoth:GetChecked() then
				data.needBoth = false
			else
				data.needBoth = true
			end
...
function WCD:OpenBodygroupsHelper(model, current, parent)
	if self.BodygroupsHelper and IsValid(self.BodygroupsHelper) then self.BodygroupsHelper:Remove() end
	...
	function a:Paint(w, h)
		surface.SetDrawColor(c.frameBg)
		surface.DrawRect(0, 0, w, h)
		draw.SimpleTextOutlined("Bodygroups Helper", "WCD::FontFrameSubTitle", w / 2, 5, color_white, TEXT_ALIGN_CENTER, 0, 1, color_black)
		surface.SetDrawColor(color_black)
		surface.DrawOutlinedRect(0, 0, w, h)
	end
	...
	function a.save:DoClick()
		if parent and IsValid(parent) then parent.value = values end
		WCD:Notification(WCD.Lang.choiceSaved, WCD.AdminUI)
		a:Remove()
	end
end
...
function WCD:OpenColorHelper(model, color, buttonToReceiveValue)
	if self.ColorHelper and IsValid(self.ColorHelper) then self.ColorHelper:Remove() end
	model = model or "models/tdmcars/242turbo.mdl"
	...
	function a:Paint(w, h)
		surface.SetDrawColor(c.frameBg)
		surface.DrawRect(0, 0, w, h)
		draw.SimpleTextOutlined("Color Helper", "WCD::FontFrameSubTitle", w / 2, 5, color_white, TEXT_ALIGN_CENTER, 0, 1, color_black)
		surface.SetDrawColor(color_black)
		surface.DrawOutlinedRect(0, 0, w, h)
	end
	...
	function a.color:ValueChanged(x)
		a.model:SetColor(x)
		if buttonToReceiveValue and IsValid(buttonToReceiveValue) then buttonToReceiveValue:SetButtonColor(a.model:GetColor()) end
	end
	...
	a.save:Dock(BOTTOM)
	function a.save:DoClick()
		if buttonToReceiveValue and IsValid(buttonToReceiveValue) then
...
function WCD:OpenSkinHelper(model, current, buttonToReceiveValue)
	if self.SkinHelper and IsValid(self.SkinHelper) then self.SkinHelper:Remove() end
	model = model or "models/tdmcars/242turbo.mdl"
	...
	function a:Paint(w, h)
		surface.SetDrawColor(c.frameBg)
		surface.DrawRect(0, 0, w, h)
		draw.SimpleTextOutlined("Skin Helper", "WCD::FontFrameSubTitle", w / 2, 5, color_white, TEXT_ALIGN_CENTER, 0, 1, color_black)
		surface.SetDrawColor(color_black)
		surface.DrawOutlinedRect(0, 0, w, h)
	end
	...
	function a:Click(forward, force)
		if force and force != 0 then
			...
			if forward and current < maxSkins then
				current = current + 1
			else
				current = current - 1
			end
		...
		a.text:SetPos(a:GetWide() / 2 - a.text:GetWide() / 2, a:GetTall() - a.right:GetTall() * 2)
	...
	a.save:Dock(BOTTOM)
	function a.save:DoClick()
		if buttonToReceiveValue and IsValid(buttonToReceiveValue) then buttonToReceiveValue.value = a.model.Entity:GetSkin() end
		WCD:Notification(WCD.Lang.choiceSaved, WCD.AdminUI)
		a:Remove()
	end
end
...
function WCD:Notification(text, popup)
	...
	for i, v in pairs(self.Notifications) do
		if not IsValid(v) then self.Notifications[i] = nil end
	end
	...
	frame.Think = function(self) if IsValid(self) and self.RemoveAt <= CurTime() or (popup and not IsValid(popup)) then self:Remove() end end
