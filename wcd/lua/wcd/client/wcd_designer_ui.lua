...
File Path:   addons/wcd/lua/wcd/client/wcd_designer_ui.lua
...
function WCD:OpenDesigner(ent)
	if self.DesignerUI and IsValid(self.DesignerUI) then self.DesignerUI:Remove() end
	local lang = self.Lang.designer
	local veh = ent or LocalPlayer():GetVehicle()
	local id = veh:GetNW2Int("WCD::Id", 0)
	if id == 0 and IsValid(LocalPlayer():GetVehicle()) and IsValid(LocalPlayer():GetVehicle():GetParent()) then id = LocalPlayer():GetVehicle():GetParent():GetNW2Int("WCD::Id", 0) end
	if not self:RetrieveAllowedCustomizations(id) then
	...
	function a:OnRemove()
		...
		if IsValid(veh) and not a.paid then
			if old.skin then veh:SetSkin(old.skin) end
			if old.color then veh:SetColor(old.color) end
			if old.underglow then veh:WCD_SetUnderglow(old.underglow) end
		...
		if WCD.CursorIsShown then gui.EnableScreenClicker(false) end
	end

	a.nextDistanceThink = CurTime()
	function a:Think()
		if not IsValid(veh) or not IsValid(LocalPlayer():GetVehicle() or LocalPlayer():GetVehicle() != veh) then
			a:OnRemove()
			a:Remove()
			return
	...
	a:SetSize(ScrW(), 60)
	a:SetPos(0, ScrH() + 60)
	a:MoveTo(0, ScrH() - 60, 0.5, 0, -1, function() if not IsValid(a) then return end end)
	a.children.finish = vgui.Create("EditablePanel")
	a.children.finish:SetSize(310, 60)
	a.children.finish:SetPos(ScrW() / 2 - a.children.finish:GetWide() / 2, ScrH() + 60)
	...
	if sp and type(sp) == "table" and table.Count(sp) > 0 then
		a.children.resetToDefault = vgui.Create("EditablePanel")
		a.children.resetToDefault:SetSize(310, 60)
		a.children.resetToDefault:SetPos(ScrW() - a.children.resetToDefault:GetWide() / 2, ScrH() + 60)
		...
		a.resetToDefault.DoClick = function()
			if a.resetToDefault.clicked or (not WCD.Settings.fullResetCost or WCD.Settings.fullResetCost < 1) then
				a.resetToDefault.clicked = nil
				if not LocalPlayer():canAfford(WCD.Settings.fullResetCost) then
	...
	if veh.GetBodyGroups then
		for i, v in pairs(veh:GetBodyGroups()) do
			if v.submodels and #v.submodels > 0 then
				showBodygroup = true
				break
			end
		end
	...
	for i, v in pairs(lang.buttons) do
		if not allowed[v.key] then continue end
		if (v.key == "skin" and (not veh.GetSkin or veh:SkinCount() == 1)) or (v.key == "color" and not veh.GetColor) or (v.key == "bodygroups" and not showBodygroup) or (v.key == "underglow" and not veh.WCD_SetUnderglow) or (v.key == "nitro" and not veh.WCD_SetNitro) then continue end
		...
		function btn:DoClick()
			...
			if c then
				...
				if c:IsVisible() then
					c:MoveTo(x + self:GetWide() / 2 - c:GetWide() / 2, ScrH(), 0.3, 0, -1, function() c:SetVisible(false) end)
					return
				end

				c:SetPos(x + self:GetWide() / 2 - c:GetWide() / 2, ScrH() - self:GetTall())
				x, y = c:GetPos()
				c:MoveTo(x, 0, 0.3)
				c:SetVisible(true)
			end
	...
	local paintFunc = function(self, w, h)
		surface.SetDrawColor(c.frameBg)
		surface.DrawRect(0, 0, w, h)
		draw.SimpleTextOutlined(self.title, "WCD::FontGenericSmall", 3, 3, color_white, 0, 0, 1, color_black)
		...
		draw.SimpleTextOutlined(WCD:Translate(m, self.price), "WCD::FontGenericSmall", w - 3, 3, color_white, TEXT_ALIGN_RIGHT, 0, 1, color_black)
	end

	local skin = a.children["skin"]
	if skin and veh.GetSkin then
		...
		function skin:GetValues()
			return skin.currentSkin != old.skin and skin.currentSkin
		end
		...
		skin.right:SetPos(skin:GetWide() - skin.right:GetWide() - 3, skin:GetTall() - skin.left:GetTall() - 3)
		...
		skin.skin:SetPos(skin:GetWide() / 2 - skin.skin:GetWide() / 2, skin:GetTall() / 2 + skin.skin:GetTall() / 2 - 5)
		...
		function skin:update()
			...
			skin.skin:SizeToContents()
			skin.skin:SetPos(skin:GetWide() / 2 - skin.skin:GetWide() / 2, skin:GetTall() / 2 + skin.skin:GetTall() / 2 - 5)
			if skin.right:IsVisible() and skin.currentSkin >= skin.maxSkins then
				skin.right:SetVisible(false)
			elseif not skin.right:IsVisible() and skin.currentSkin < skin.maxSkins then
				skin.right:SetVisible(true)
			end

			if skin.currentSkin == 0 then
				skin.left:SetVisible(false)
			elseif not skin.left:IsVisible() and skin.currentSkin > 0 then
			...
			if skin.currentSkin == old.skin then
				...
				if not a.hasBeenPriced["skin"] then
					a.total = a.total + data:GetSkinCost()
	...
	if color and veh.SetColor then
		...
		function color:GetValues()
			...
			return (a.r != old.color.r or a.b != old.color.b or a.c != old.color.c) and a or nil
		end
		...
		function color.picker:ValueChanged(x)
			veh:SetColor(x)
			if not a.hasBeenPriced["color"] then
	...
	if bg and veh.GetBodyGroups then
		old.bodygroups = {}
		for i, v in pairs(veh:GetBodyGroups()) do
		...
		function bg:GetValues()
			return table.Count(edits) > 0 and edits
		end
		...
		bg.layout:Dock(FILL)
		for i, v in pairs(veh:GetBodyGroups()) do
			if not v.submodels or table.Count(v.submodels) < 2 then continue end
			...
			if table.Count(v.submodels) == 2 then
				...
				function q.choice:OnSelect(choice)
					...
					if not a.hasBeenPriced["bodygroups"][v.id] then
						a.hasBeenPriced["bodygroups"][v.id] = true
						a.total = a.total + data:GetBodygroupCost()
	...
	if underglow and veh.WCD_GetUnderglow then
		...
		function underglow:GetValues()
			...
			return (a.r != b.r or a.g != b.g or a.b != b.b) or nil
		end
		...
		function underglow.picker:ValueChanged(x)
			if not a.hasBeenPriced["underglow"] then
	...
	if nitro and veh.WCD_GetNitro then
		...
		local prices = {data:GetNitroOneCost(), data:GetNitroTwoCost(), data:GetNitroThreeCost()}
		local buttons = {}
		...
		for i = 1, 3 do
			...
			function q.button:DoClick()
				...
				if a.hasBeenPriced["nitro"] and veh.__DesignNitro then
				...
				for i, v in pairs(buttons) do
					...
					v.active = false
					if i == old.nitro and not veh.__DesignNitro then
						v:DoClick()
					else
			...
			if i == old.nitro then
				...
				function q.button:DoClick()
					...
					if veh.__DesignNitro == 0 then
						...
						veh.__DesignNitro = old.nitro
						for i, v in pairs(buttons) do
							if v != self then v:SetButtonColor(c.mainColor) end
						end
					else
						if veh.__DesignNitro and veh.__DesignNitro != 0 then
	...
	function a.finish:DoClick()
		...
		for i, v in pairs(a.children) do
			if i == "finish" or i == "resetToDefault" or type(v:GetValues()) == "nil" or not v:GetValues() then continue end
...
net.Receive("WCD::SpawnAndCustomize", function()
	...
	timer.Simple(0.5, function() WCD:OpenDesigner(Entity(ent)) end)
end)

hook.Add("HUDPaint", "WCD::CursorStatus", function()
	if IsValid(WCD.DesignerUI) then
		draw.SimpleTextOutlined(WCD.Lang.designer.spaceToToggleMouse, "WCD::FontDealer", ScrW() / 2, ScrH() - 150, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, color_black)
		draw.SimpleTextOutlined(WCD:Translate(WCD.Lang.designer.totalPrice, DarkRP.formatMoney(WCD.DesignerUI.total)), "WCD::FontDealer", ScrW() / 2, ScrH() - 250, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, color_black)
		draw.SimpleTextOutlined(WCD:Translate(WCD.Lang.dealerVarious.wallet, DarkRP.formatMoney(LocalPlayer():getDarkRPVar("money"))), "WCD::FontDealer", ScrW() / 2, ScrH() - 280, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, color_black)
		if WCD.DesignerUI.children["underglow"] and WCD.DesignerUI.children["underglow"]:IsVisible() then draw.SimpleTextOutlined(WCD.Lang.designer.underglow, "WCD::FontDealer", ScrW() / 2, ScrH() - 190, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, color_black) end
	end
end)

local last = CurTime()
hook.Add("KeyPress", "WCD::ToggleCursor", function(ply, key)
	if key == IN_JUMP and IsValid(WCD.DesignerUI) and last < CurTime() then
		WCD.CursorIsShown = not WCD.CursorIsShown
