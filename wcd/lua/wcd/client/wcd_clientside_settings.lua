...
File Path:   addons/wcd/lua/wcd/client/wcd_clientside_settings.lua
...
function WCD:LoadClientSettings()
	if not file.Exists("wcd", "DATA") then file.CreateDir("wcd") end
	if not file.Exists("wcd/favorites/", "DATA") then file.CreateDir("wcd/favorites/") end
	...
	if file.Exists("wcd/favorites/" .. self.Settings.serverString .. ".txt", "DATA") then self.Client.Favorites = util.JSONToTable(file.Read("wcd/favorites/" .. self.Settings.serverString .. ".txt", "data")) end
end

function WCD:OpenClientSettings(func)
	if self.Client.Frame and IsValid(self.Client.Frame) then self.Client.Frame:Remove() end
	local c = self.Colors
	if not WCD.Languages[self.Client.Settings.Language] then self.Client.Settings.Language = "english" end
	...
	function a:Paint(w, h)
		...
		draw.SimpleTextOutlined("Clientside Settings", "WCD::FontFrameSubTitle", w / 2, 5, color_white, TEXT_ALIGN_CENTER, 0, 1, color_black)
	...
	for i, v in pairs(lang.data) do
		...
		elseif v.type == "combobox" then
			...
			if type(WCD.Client.Settings[v.key]) == "number" then
				...
				q.choice:SetText(choices[string.lower(WCD.Client.Settings[v.key] or "Unknown")] or "Unknown")
			end
