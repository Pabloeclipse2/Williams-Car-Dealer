...
File Path:   addons/wcd/lua/entities/wcd_platform/cl_init.lua
...
function ENT:Think()
	if WCD.__StopDealer then
		if WCD.__StopDealer >= CurTime() and self:GetNoDraw() then
			self:SetNoDraw(false)
		elseif WCD.__StopDealer <= CurTime() and not self:GetNoDraw() then
			self:SetNoDraw(true)
		end
	end
end
...
function ENT:Draw()
	...
	local dir = _e:GetUp()
	local len = 128
	local dealer = Entity(self:GetNW2Int("WCD::DealerID"))
	...
	draw.SimpleTextOutlined(WCD.Lang.visibleFor .. math.Round(WCD.__StopDealer - CurTime()) .. "s", "WCD::FontVeryLarge", 0, 0, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 5, color_black)
	cam.End3D2D()
