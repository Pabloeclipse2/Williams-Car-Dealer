...
File Path:   addons/wcd/lua/weapons/wcd_admin/shared.lua
...
function SWEP:Initialize()
	if CLIENT and self:GetOwner() == LocalPlayer() then WCD.__StopDealer = CurTime() + 30 end
	if SERVER then self:GetOwner().__WCDCanPhys = CurTime() + 60 end
	self:SetHoldType("pistol")
end

function SWEP:Deploy()
	if CLIENT and self:GetOwner() == LocalPlayer() then
		WCD.__StopDealer = CurTime() + 30
		LocalPlayer():ChatPrint(lang.instructions)
	end
...
function SWEP:PrimaryAttack()
	...
	if not WCD:IsOwner(self:GetOwner()) then
		if CLIENT then WCD:Notification(lang.invalidRank) end
		return
	end
	...
	if tr and tr.Entity then
		...
		if tr.Entity:GetClass() == "wcd_dealer" then
			if CLIENT then WCD:OpenDealerMenu(_e) end
		...
		elseif tr.Entity == game.GetWorld() then
			...
			if CLIENT then WCD:Notification(lang.aimHelp) end
		end
	end
end
...
function SWEP:SecondaryAttack()
	...
	if tr and tr.Entity then
		...
		if tr.Entity:GetClass() == "wcd_pump" then
			if SERVER then
				_e:Remove()
				timer.Simple(1, function() WCD:Save("pumps") end)
			else
...
if CLIENT then
	...
	function WCD:OpenDealerMenu(dealer)
		if self.DealerMenu and IsValid(self.DealerMenu) then self.DealerMenu:Remove() end
		...
		function a:Paint(w, h)
			...
			draw.SimpleTextOutlined("Dealer Configuration", "WCD::FontFrameSubTitle", w / 2, 5, color_white, TEXT_ALIGN_CENTER, 0, 1, color_black)
		...
		for i, v in pairs(lang.settings) do
			if i == "globalReturn" and not WCD.Settings.canOnlyReturnSpawned then continue end
			...
			if v.values then
				...
				q.choice:SetText(ref[dealer:GetNW2Float("group", -1)] or WCD.Lang.none)
			...
			elseif v.type == "string" then
				...
				elseif v.key == "name" then
					q.choice:SetDefaultText(dealer:GetNW2String("WCD::Name", "Unknown Name"))
				end
			elseif v.type == "checkbox" then
				q.choice = q:Add("WCD::DCheckBox")
				q.choice:SetSize(24, 24)
				q.choice:SetPos(q:GetWide() - q.choice:GetWide() - 1, q:GetTall() / 2 - q.choice:GetTall() / 2)
				if v.nwbool then q.choice:SetChecked(dealer:GetNW2Bool("WCD::" .. v.key)) end
				function q.choice:GetValue()
			...
			elseif v.type == "button" then
				...
				elseif v.key == "deleteAllPlatform" then
					...
					for i, v in pairs(ents.FindByClass("wcd_platform")) do
						if v:GetNW2Int("WCD::DealerID", 0) == dealer:EntIndex() then count = count + 1 end
					end
			...
			if v.type != "button" then values[v.key] = q.choice end
		end
